...
PROVIDER="${IMAGE_PROVIDER:-nano-banana}"
PROMPT=""
ASPECT_RATIO="1:1"
...
Options:
  --provider NAME          nano-banana or openai. Default: nano-banana.
  ...
  --size SIZE              OpenAI image size. Default: 1024x1024.
...
save_inline_images_from_json() {
  ...
  jq -n --arg provider "$PROVIDER" --argjson images "$images_json" --arg raw_json "$(absolute_path "$response_path")" \
    '{status: "success", provider: $provider, images: $images, raw_json: $raw_json}'
}
...
generate_nano_banana() {
  local api_url="${NANO_BANANA_API_URL:-${NANOBANANA_API_URL:-}}"
  local api_key="${NANO_BANANA_API_KEY:-${NANOBANANA_API_KEY:-}}"

  if [[ -z "$api_url" ]]; then
    echo "Missing Nano Banana API URL. Set NANO_BANANA_API_URL." >&2
    exit 1
  fi
  if [[ -z "$api_key" ]]; then
    echo "Missing Nano Banana API key. Set NANO_BANANA_API_KEY." >&2
    exit 1
  fi
...
case "$PROVIDER" in
  ...
  openai|gpt-image|gpt-image-2|chatgpt-image)
    ...
    ;;
  *)
    echo "Unsupported provider: $PROVIDER. Use nano-banana or openai." >&2
    exit 1
    ;;
esac
